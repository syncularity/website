...
The selected Mercury header uses a right-facing play symbol built from ASCII characters, with a gentle 12-second CSS transform animation. Regenerate its SVG with `node scripts/generate-header-mark.mjs`. It follows the artwork's shared motion state, including pause, hidden tabs and offscreen suspension, and remains static with reduced motion or without JavaScript.
